...
async def create_card(card_data: CardCreate, db: Session = Depends(get_db), current_user: User = Depends(get_current_user)):
    """创建充值卡（管理员权限）"""
    def _create():
...
async def list_cards(include_inactive: bool = False, db: Session = Depends(get_db), current_user: User = Depends(get_current_user)):
    """获取充值卡列表（管理员权限）"""
    def _list():
...
async def get_card(card_id: int, db: Session = Depends(get_db), current_user: User = Depends(get_current_user)):
    """获取单个充值卡（管理员权限）"""
    def _get():
...
async def update_card(card_id: int, card_data: CardUpdate, db: Session = Depends(get_db), current_user: User = Depends(get_current_user)):
    """更新充值卡（管理员权限）"""
    def _update():
...
async def get_card_stock(card_name: str, db: Session = Depends(get_db), current_user: User = Depends(get_current_user)):
    """获取充值卡库存数量（管理员权限）"""
    def _get_stock():
