...
from src.server.auth.router.admin import (
    admin_create_user,
    admin_get_users,
    admin_get_user,
    admin_update_user,
...
from src.server.auth.schemas import (
    Role,
    UserListResponse,
    UserProfile,
    AdminUserCreate,
...
async def get_users(
    ...
    """获取用户列表（转发到 auth 模块）"""
    return await admin_get_users(role, page, page_size, current_admin, db)
...
async def create_user(
    ...
    """创建用户（转发到 auth 模块）"""
    return await admin_create_user(user_data, current_admin, db)


@router.get("/users/{user_id}", response_model=UserProfile, summary="获取指定用户")
async def get_user(
    user_id: int,
    current_admin: User = Depends(get_current_admin),
    db: Session = Depends(get_db),
    ...
    """获取指定用户（转发到 auth 模块）"""
    return await admin_get_user(user_id, current_admin, db)


@router.put("/users/{user_id}", response_model=UserProfile, summary="更新指定用户")
async def update_user(
    ...
    """更新指定用户（转发到 auth 模块）"""
    return await admin_update_user(user_id, user_data, current_admin, db)
...
async def delete_user(
    ...
    """删除指定用户（转发到 auth 模块）"""
    return await admin_delete_user(user_id, current_admin, db)
